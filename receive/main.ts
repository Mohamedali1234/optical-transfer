...
function finish(payload: Uint8Array, hashOk: boolean, seconds: number, totalLen: number) {
  ...
  // UNIVERSAL FILE DOWNLOAD LINK (Supports any file type: PDFs, zips, code, apps, images)
  const blob = new Blob([payload as BlobPart]);
  const downloadLink = document.createElement("a");
  downloadLink.href = URL.createObjectURL(blob);
  downloadLink.download = currentFileName;
  downloadLink.className = "download-btn"; // Hooks into existing style classes
  downloadLink.textContent = `Download ${currentFileName}`;
  ...
  result.append(heading, downloadLink);
  
  // Automatically trigger the download download
  downloadLink.click();
}
